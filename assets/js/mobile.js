(function($){
    ...
    function getSerial() {
        $.get('http://192.168.33.41:3000/get/json/all', function(data){
            ...
            Mustache.parse(template);
            var rendered = Mustache.render(template, {temp: json_decoded['degrees'] - 5, light: Math.round((json_decoded['light'] / 5.00) * 100), voltage: json_decoded['tension'], door: Math.round((json_decoded['door'] / 59) * 100), alarm: json_decoded['a_status'], buzzer: json_decoded['b_status'], user : user['login'], alarm_state : json_decoded['alarm'], blue : blue, white : white, green : green, red : red, time : time});
